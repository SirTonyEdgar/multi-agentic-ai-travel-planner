...
const HotelDetailPage = () => {
  ...
  const handleProceed = () => {
    if (selectedRoom) {
      navigate(`/checkout?hotelId=${hotel.id}&room=${encodeURIComponent(selectedRoom.type)}&price=${selectedRoom.price}&query=${encodeURIComponent(queryParam)}`);
    }
  };

  const handleBack = () => {
    if (queryParam) {
      // Redirect kembali ke website Kelana
      window.location.href = `${JALAN_URL}/plan?q=${encodeURIComponent(queryParam)}`;
    } else {
      window.location.href = JALAN_URL;
    }
  };
    ...
    <div className="hotel-detail-page">
      ...
      <header className="pesan-navbar">
        <div className="pesan-navbar-inner">
          <button onClick={handleBack} className="pesan-back-link" style={{ background: 'none', border: 'none', cursor: 'pointer' }}>
            <ArrowLeft size={18} />
            <span>Kembali ke Kelana</span>
          </button>
